...
def hamiltonian_to_sparse(ham: Dict[str, float]):
    """Hamiltonian dict to sparse matrix."""
    return reduce(
        lambda x, y: x + y,
        [ham[t] * pauli_to_sparse(t) for t in ham.keys()]
    )
...
def get_reduced_hamiltonians(
    ...
    """Get reduced Hamiltonians for CS-VQE. The ith element corresponds to
    simulating i qubits on the quantum computer."""
    ...
    # Rectify order for sequential dropping
    ...
    out = []
    diag_set = list(diagonal_set)
    val_list = list(vals)

    for k in range(order_len + 1):
        ham_rotated = dict(ham)
        ...
        if order:
            i_drop = order.pop(0)
            diag_set = diag_set[:i_drop] + diag_set[i_drop + 1:]
            val_list = val_list[:i_drop] + val_list[i_drop + 1:]

    return out


def contextual_subspace_approximations(
    ham: Dict[str, float],
    model: Tuple,
    fn_form: List,
    ep_state: List,
    ...
    """Return CS-VQE energy approximations for each qubit count. 0th = pure noncontextual."""
    reduced_hamiltonians = get_reduced_hamiltonians(ham, model, fn_form, ep_state, list(order))
    n_q = len(next(iter(ham.keys())))

    out = []
    for ham_red in reduced_hamiltonians:
        if len(ham_red) == 1 and '' in ham_red:
            out.append(float(np.real(ham_red[''])))
        else:
            ham_red_sparse = hamiltonian_to_sparse(ham_red)
            n_red = len(next(iter(ham_red.keys()))) if ham_red else 0
            if n_red <= 4:
                out.append(float(np.min(np.linalg.eigvalsh(ham_red_sparse.toarray()))))
            else:
                out.append(float(sp.sparse.linalg.eigsh(ham_red_sparse, which='SA', k=1)[0][0]))

    return out


def csvqe_approximations_heuristic(
    ham: Dict[str, float],
    ham_noncon: Dict[str, float],
    n_qubits: int,
    true_gs: float,
) -> List:
    """Heuristic qubit ordering for CS-VQE. Returns [true_gs, approxs, errors, order]."""
    model = quasi_model(ham_noncon)
    fn_form = energy_function_form(ham_noncon, model)
    gs_noncon = find_gs_noncon(ham_noncon, method='differential_evolution')

    if gs_noncon[0] - true_gs < 1e-10:
        return [true_gs, [gs_noncon[0]] * n_qubits, [gs_noncon[0] - true_gs] * n_qubits, list(range(n_qubits))]
    ...
    while len(indices) > 1 and not exact:
        ...
        for i_subset in itertools.combinations(indices, num_to_remove):
            ...
            if errors_temp[current_i] < 1e-6:
                exact = True
            improvement = errors_temp[current_i - 2] - errors_temp[current_i]
            i_subset_improvements[i_subset] = i_subset_improvements.get(i_subset, 0) + improvement

        best_i_subset = max(i_subset_improvements, key=i_subset_improvements.get)
        order = order + list(best_i_subset)
        indices = [i for i in range(n_qubits) if i not in order]

    order_full = list(order)
    for i in range(n_qubits):
        if i not in order_full:
            order_full.append(i)
